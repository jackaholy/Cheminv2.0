...
def search_route():
    ...
    search_terms = [search_term for search_term in search_terms for synonym in search_term if len(synonym) > 3 or synonym == query]
    
    matching_chemicals = []
    for search_term in search_terms:
        matches = db.session.query(Chemical).filter(
            or_(
    ...
    response_entries = [
        {"name": chemical.Chemical_Name, "symbol": chemical.Chemical_Formula}
