...
def test_add_chemical_duplicate_product_number(client):
    """
    Test that adding a chemical with a duplicate product number is allowed
    if the manufacturer is different, but causes an error if the manufacturer
    is the same.
    """
    # Use existing manufacturers and storage class from testdata
    ...
    # Use an existing chemical and create a Chemical_Manufacturer record
    chemical = db.session.query(Chemical).filter_by(Chemical_Name="Acetone").first()
    chem_manufacturer = Chemical_Manufacturer(
        Chemical=chemical, Manufacturer=manufacturer1, Product_Number="T123"
    )
    db.session.add(chem_manufacturer)
    db.session.commit()

    # Test adding a chemical with the same product number but a different manufacturer
    payload_different_manufacturer = {
        "chemical_name": "Test Chemical",
        "chemical_formula": "H2O",
        "product_number": "T123",  # Duplicate product number
        "storage_class_id": storage_class.Storage_Class_ID,
        "manufacturer_id": manufacturer2.Manufacturer_ID,  # Different manufacturer
    }
    response = client.post(
        "/api/add_chemical",
        data=json.dumps(payload_different_manufacturer),
        content_type="application/json",
    )
    ...
    # Test adding a chemical with the same product number and the same manufacturer
    payload_same_manufacturer = {
        "chemical_name": "Test Chemical",
        "chemical_formula": "H2O",
        "product_number": "T123",  # Duplicate product number
        "storage_class_id": storage_class.Storage_Class_ID,
        "manufacturer_id": manufacturer1.Manufacturer_ID,  # Same manufacturer
    }
    response = client.post(
        "/api/add_chemical",
        data=json.dumps(payload_same_manufacturer),
        content_type="application/json",
    )
