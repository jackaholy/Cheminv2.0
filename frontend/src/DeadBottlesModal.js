...
const DeadBottlesModal = ({ show, handleClose, refreshChemicals }) => {
  ...
  const deleteDeadBottles = async () => {
    try {
      const response = await fetch("/api/chemicals/delete_dead", {
        ...
        body: JSON.stringify({
          dead_bottles: deadChemicals
            .map((chemical) => chemical.inventory.map((bottle) => bottle.id))
            .flat(),
        }),
        credentials: "include",
      });
  ...
  const sortedChemicals = deadChemicals
    .flatMap((chemical) =>
  ...
  return (
    <>
      <Modal show={show} onHide={handleClose} size="xl" centered scrollable>
        ...
        <Modal.Body>
          {error ? (
            ...
            <>
              <p className="my-1">
                The following bottles haven't been updated in 2+ years and are
                slated for deletion. Do you want to permanently and irreversably
                delete all of them?
              </p>
              <button
                className="btn btn-danger pad-danger my-1"
                    ...
                    <tr key={bottle.sticker}>
                      ...
                      <td>
                        <OverlayTrigger
                          placement="bottom"
                          overlay={
                            <Tooltip id="mark-alive">Mark as alive</Tooltip>
                          }
                        >
                          <button
      ...
      >
        ...
        <Modal.Body>
          Are you sure you want to permanently delete all dead bottles (that
          haven't been updated in 2+ years)? This action cannot be undone.
        </Modal.Body>
        <Modal.Footer>
          <Button
            variant="secondary"
