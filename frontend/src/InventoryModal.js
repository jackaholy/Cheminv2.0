...
export const InventoryModal = ({show, handleClose: parentHandleClose}) => {
    ...
    const handleKeyDown = async (e) => {
        if (e.key === "Enter") {
            ...
            if (currentTime - lastEnterTimeRef.current <= DOUBLE_ENTER_THRESHOLD) {
                ...
                setStatusMessage(`Entered Sticker Number: ${sticker_number}`);
                    ...
                    try {
                        ...
                        if (
                            ...
                            if (confirmMove) {
                                ...
                                setStatusMessage("Location updated successfully."); // Update status message
                                // Reload list
                        ...
                        setStatusMessage(`Error: ${err.message}`); // Update status message on error
                    }
                }
    ...
    const handleCompleteSublocation = () => {
        ...
        if (unenteredChemicals.length === 0) {
            setStatusMessage("No chemicals left to be marked dead."); // Update status message
            return;
        }
        ...
        fetch("/api/chemicals/mark_many_dead", {
            ...
            headers: {"Content-Type": "application/json"},
            body: JSON.stringify({
                inventory_id: unenteredChemicals,
            ...
            .then((data) => setStatusMessage(data.message)) // Update status message
            .catch((error) => {
                console.error("Error marking chemicals as dead:", error);
                setStatusMessage(`Error: ${error.message}`); // Update status message on error
            });
    };
    ...
    return (
        <Modal show={show} onHide={handleClose} centered size="lg">
                            ...
                            if (selectedSubLocation) {
                                fetch(`/api/chemicals/by_sublocation?sub_location_id=${selectedSubLocation.sub_location_id}`)
                                    ...
                                    .then((data) => {
                                        ...
                                        setEnteredChemicals(new Set());
                                        setInputValue("");
                                        setStatusMessage("Reset successful."); // Update status message
                                    })
                                    .catch((error) => {
            ...
            {/* Status message below the footer */}
            {statusMessage && (
                <div className="px-4 pb-3 text-muted small text-center">
                    {statusMessage}
                </div>
            )}
        </Modal>
