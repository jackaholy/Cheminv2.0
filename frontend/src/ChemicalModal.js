...
export const ChemicalModal = ({
  ...
  async function markDead(item) {
    try {
      ...
      refreshChemicals();
      setStatusMessage(`Marked bottle #${item.sticker} (${item.location}, ${item.sub_location}) as dead`);
  ...
  async function markAlive(item) {
    try {
      ...
      const data = await response.json();
      refreshChemicals();
      setStatusMessage(`Marked bottle #${item.sticker} (${item.location}, ${item.sub_location}) as alive`);
        ...
        <div className="container">
          <div className="row">
            ...
            <div className="col-md-8">
              <p><b>Storage Class:</b> {chemical.storage_class}</p>
              <p><b>Chemical Formula:</b> {chemical.formula}</p>
              <p>
        ...
        <table className="table mb-2">
          <thead>
            <tr>
              ...
              <th>Sub-Location</th>
              <th>Manufacturer</th>
              {["Editor", "Full Access"].includes(user.access) && <th></th>}
            </tr>
          </thead>
          <tbody>
